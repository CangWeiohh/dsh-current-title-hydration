...
- 保持 `package.json` 的 `dsh.bundle.patch` 配置、包名 `dsh-current-title-hydration` 与 `cordis.patch.yml` 中的插件 ID 一致。
- 修改插件代码后先运行 `npm run check`；不要将生成的安装产物、profile 文件或 DSH_HOME 数据提交到仓库。
- 该插件仅为当前选中的会话补充标题投影；不要扩展为批量读取全部会话，也不要修改 DSH 核心 bundle、会话日志、projection-cache 或工作区数据。
